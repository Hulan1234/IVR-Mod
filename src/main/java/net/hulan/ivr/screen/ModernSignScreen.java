...
public class ModernSignScreen extends ScreenMapper implements IGui {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        try {
            ...
            for (int i = 0; i < signIds.length; i++) {
                if (signIds[i] != null) {
                    RenderModernSign.drawSign(matrices, null, null, textRenderer, signPos, signIds[i], (width - SIGN_SIZE * length) / 2F + i * SIGN_SIZE, 0, SIGN_SIZE, RenderModernSign.getMaxWidth(signIds, i, false), RenderModernSign.getMaxWidth(signIds, i, true), selectedIds, Direction.UP, 0, (textureId, x, y, size, flipTexture) -> {
            ...
            if (editingIndex >= 0) {
                ...
                loopSigns((index, x, y, isBig) -> {
                    ...
                    if (sign != null) {
                        ...
                        RenderModernSign.drawSign(matrices, null, null, textRenderer, signPos, signId, (isBig ? xOffsetBig : xOffsetSmall) + x + (moveRight ? SIGN_BUTTON_SIZE * 2 : 0), BUTTON_Y_START + y, SIGN_BUTTON_SIZE, 2, 2, selectedIds, Direction.UP, 0, (textureId, x1, y1, size, flipTexture) -> drawTexture(matrices, (int) x1, (int) y1, 0, 0, (int) size, (int) size, (int) (flipTexture ? -size : size), (int) size));
                    }
                }, false);
    ...
    private int loopSigns(ModernSignScreen.LoopSignsCallback loopSignsCallback, boolean ignorePage) {
