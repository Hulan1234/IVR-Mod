...
public class KSDRailwayDataLoggingModule {

    private final Path logsPath;
    private final Path filePath;
    private final List<String> queuedEvents = new ArrayList<>();

    public KSDRailwayDataLoggingModule(Path savePath) {
        logsPath = savePath.resolve("logs");
        SimpleDateFormat var10002 = new SimpleDateFormat("yyyyMMdd-HHmmssSSS");
        Date var10003 = new Date();
        filePath = logsPath.resolve(var10002.format(var10003) + ".csv");
    }
        ...
        if (!queuedEvents.isEmpty()) {
            try {
                if (!Files.exists(filePath)) {
                    ...
                    queuedEvents.add(0, String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s", "Timestamp", "Player Name", "Player UUID", "Class", "ID", "Name", "Position", "Change", "Old Data", "New Data"));
    ...
    public void addEvent(ServerPlayer player, Class<?> dataClass, List<String> oldData, List<String> newData, BlockPos... positions) {
        addEvent(player, dataClass, 0L, "", oldData, newData, positions);
    }
    ...
    public void addEvent(ServerPlayer player, Class<?> dataClass, long id, String name, List<String> oldData, List<String> newData, BlockPos... positions) {
        ...
        if (oldData.size() == newData.size()) {
            ...
            for(int i = 0; i < oldData.size(); ++i) {
                ...
                String newDataString = newData.get(i);
        ...
        if (!oldDataDiff.isEmpty() || !newDataDiff.isEmpty()) {
            queuedEvents.add(String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s", formatString((new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSXXX")).format(new Date())), formatString(player.getName().getString()), formatString(player.getUUID().toString()), formatString(dataClass.getName()), formatString(id == 0L ? "" : String.format("[%s]", id)), formatString(IGui.formatStationName(name)), formatString(String.join("\n", positionsList)), formatString((oldDataDiff.isEmpty() ? LoggingEditType.CREATE : (newDataDiff.isEmpty() ? LoggingEditType.DELETE : LoggingEditType.EDIT)).toString()), formatString(RailwayData.prettyPrint(JsonParser.parseString(String.format("{%s}", String.join(",", oldDataDiff))))), formatString(RailwayData.prettyPrint(JsonParser.parseString(String.format("{%s}", String.join(",", newDataDiff)))))));
        }
    ...
    public static <T extends SerializedDataBase> List<String> getData(T data) {
        try {
            ...
            try (MessageBufferPacker messageBufferPacker = MessagePack.newDefaultBufferPacker()) {
                ...
                try (MessageUnpacker messageUnpacker = MessagePack.newDefaultUnpacker(messageBufferPacker.toByteArray())) {
                    List<String> dataList = new ArrayList<>();
                    String previousData = null;

                    while(messageUnpacker.hasNext()) {
                        String newValue = messageUnpacker.unpackValue().toJson();
    ...
    public static List<String> getData(CompoundTag compoundTag) {
        ...
        compoundTag.getAllKeys().forEach((key) -> {
            String value = convertTag(compoundTag.get(key));
            if (value != null) {
    ...
    private static String convertTag(Tag tag) {
        if (tag instanceof CollectionTag) {
            List<String> valueTags = new ArrayList<>();
            ((CollectionTag) tag).forEach((data) -> valueTags.add(convertTag((Tag) data)));
            return String.format("[%s]", String.join(",", valueTags));
        } else if (tag != null) {
            String tempValue = tag.getAsString();
