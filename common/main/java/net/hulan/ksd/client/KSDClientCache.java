...
public class KSDClientCache extends KSDDataCache {
    ...
    private static final int MAX_DYNAMIC_TEXTURE_SIZE = 4096; // 限制动态纹理尺寸，避免超过显卡支持范围。
            ...
            if (resourcesInFlight.add(key)) { // 仅允许同一资源同时存在一个后台生成任务。
                CompletableFuture.supplyAsync(supplier).whenComplete((nativeImage, throwable) -> resourceRegistryQueue.add(() -> { // 异步生成图像并把注册工作排回主线程。
                    ...
                    if (throwable != null || nativeImage == null // 处理异步生成失败或没有生成图像的情况。
                            || nativeImage.getWidth() <= 0 || nativeImage.getHeight() <= 0
                            || nativeImage.getWidth() > MAX_DYNAMIC_TEXTURE_SIZE
                            || nativeImage.getHeight() > MAX_DYNAMIC_TEXTURE_SIZE) { // 拒绝非法或超大纹理，避免触发 OpenGL 错误。
