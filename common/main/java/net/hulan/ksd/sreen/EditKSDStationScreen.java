...
public class EditKSDStationScreen extends EditNameColorScreenBase<KSDStation> {
    ...
    public void tick() {
        ...
        List<DataConverter> exitParents = DataUtilities.getSortedAndMappedListFromDataCollection(data.exits.keySet(), value -> {
        ...
        List<DataConverter> exitDestinations = parentExists() ? DataUtilities.getMappedListFromDataCollection(data.exits.get(editingExit), value -> new DataConverter(value, 0)) : new ArrayList<>();
    ...
    private void executeStationUpdate(Consumer<Station> action) {
        DataUtilities.executeFromDataSet(ClientData.STATIONS, s -> s.id == data.id, action);
    }
}
