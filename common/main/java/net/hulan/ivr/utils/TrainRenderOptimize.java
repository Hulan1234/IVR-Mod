...
public final class TrainRenderOptimize {
    ...
    public static boolean shouldCullBlockEntity(BlockPos pos, Vec3 rel, double radius) {
        ...
        if (previousRequest == null || now - previousRequest >= BLOCK_ENTITY_VISIBILITY_REFRESH_NANOS) {
            ...
            final boolean[] blockedSamples = captureBlockEntityOcclusionSnapshot(minecraft.level, cameraPosition, pos, radius); // 在当前线程读取遮挡采样快照。
            OCCLUSION_EXECUTOR.execute(() -> { // 将视锥和遮挡结果汇总交给后台线程。
                final boolean cull = isOutsideFrustumSnapshot(rel, radius, cameraYaw, cameraPitch, tanVertical, aspect); // 计算方块实体是否完全在视锥外。
                boolean occluded = true; // 假定方块实体完全被遮挡，直到发现可见采样点。
                for (boolean blocked : blockedSamples) { // 检查方块实体的所有遮挡采样点。
                    if (!blocked) {
                        occluded = false; // 任一采样点可见即可证明方块实体没有完全被遮挡。
                        break; // 停止检查剩余采样点。
                    }
                }
    ...
    private static boolean isRayBlocked(net.minecraft.world.level.Level world, Vec3 start, Vec3 end) {
        return isRayBlocked(world, start, end, null); // 使用默认射线检测，不排除目标方块。
    }
    ...
    private static boolean isRayBlocked(net.minecraft.world.level.Level world, Vec3 start, Vec3 end, BlockPos ignoredPos) {
        ...
        while (Math.min(nextX, Math.min(nextY, nextZ)) < 1.0D) {
            ...
            BlockPos currentPos = new BlockPos(x, y, z); // 构造射线当前经过的方块坐标。
            if (currentPos.equals(ignoredPos)) { // 跳过方块实体自身所在的方块。
                continue; // 避免把目标方块自身误判为遮挡物。
            }
            BlockState state = world.getBlockState(currentPos); // 读取射线经过的方块状态。
    ...
    private static double getFovTanHalf() {
        ...
        if (now < fovCacheExpiresAt) { // 判断视场角缓存是否仍然有效。
            return cachedFovTanHalf; // 直接返回缓存结果，避免重复反射。
        }
        try { // 尝试读取当前客户端的视场角设置。
            ...
            if (value instanceof Number) {
                cachedFovTanHalf = Math.tan(Math.toRadians(Math.max(70.0D, ((Number) value).doubleValue()) * 0.5D)); // 更新视场角一半的正切值。
                fovCacheExpiresAt = now + 500_000_000L; // 将视场角缓存保持半秒。
                return cachedFovTanHalf; // 返回更新后的视场角结果。
            }
        } catch (Exception ignored) { // 字段不可用时使用保守默认值。
        }
        fovCacheExpiresAt = now + 500_000_000L; // 缓存默认视场角，避免重复反射失败。
        return cachedFovTanHalf; // 返回缓存的保守视场角结果。
    }
