...
import net.hulan.ivr.screen.ModernSign1OddScreen;
...
public class IVRPacketTrainDataGuiClient extends PacketTrainDataBase implements IVRPacket {
    ...
    public static void openModernSign1OddScreenS2C(Minecraft minecraftClient, FriendlyByteBuf packet) {
        final BlockPos pos = packet.readBlockPos();
        minecraftClient.execute(() -> {
            if (!(minecraftClient.screen instanceof ModernSign1OddScreen)) {
                UtilitiesClient.setScreen(minecraftClient, new ModernSign1OddScreen(pos));
            }
        });
    }
    ...
    public static void sendModernSign1OddIdsC2S(BlockPos signPos, Set<Long> selectedIds1, String[] signId1, Set<Long> selectedIds2, String[] signId2, boolean luminance) {
        final FriendlyByteBuf packet = new FriendlyByteBuf(Unpooled.buffer());
        packet.writeBlockPos(signPos);
        packet.writeInt(selectedIds1.size());
        selectedIds1.forEach(packet::writeLong);
