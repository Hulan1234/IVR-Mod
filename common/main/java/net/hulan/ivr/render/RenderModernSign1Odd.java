...
public class RenderModernSign1Odd<T extends BlockModernSign.TileEntityModernSign1Odd> extends BlockEntityRendererMapper<T> implements IBlock, IGui, IDrawing {

    public RenderModernSign1Odd(BlockEntityRenderDispatcher dispatcher) {
        super(dispatcher);
    }
    ...
    private void renderSide(T entity, PoseStack matrices, MultiBufferSource vertexConsumers) {
        ...
        for (int i = 0; i < signId1.length; i++) {
            if (signId1[i] != null) {
                RenderModernSign.drawSign(matrices,
                        vertexConsumers,
